#!/usr/bin/env python
"""Full-depth Chinese academic research report (content expansion over 96).
...
Outputs (does NOT overwrite 研究报告.* or English report.*):
  完整研究报告.html / .md / .pdf

Run:
  D:\\miniforge3\\envs\\hydromodel\\python.exe scripts/99_full_report_zh.py
"""
...
from _deep_fig_zh import deep_explains, deep_glossary_html  # noqa: E402
...
HTML_ZH = ROOT / "完整研究报告.html"
MD_ZH = ROOT / "完整研究报告.md"
PDF_ZH = ROOT / "完整研究报告.pdf"

DATE_STR = "2026-08-16"
PROJECT_PATH = r"I:\Projects\202606-JOH-zonal-LSG"
SCRIPT_NAME = "scripts/99_full_report_zh.py"
...
def build(data: dict) -> tuple[str, str, list[str], list[str]]:
    ...
    figs = d["figs"]
    expl = deep_explains(d)

    def fig_block(num, title, fname, alt, explain_html, synthetic=False, conflict=False):
        # Prefer deep per-panel explanations when available
        if fname in expl:
            explain_html = expl[fname]
        b64 = figs.get(fname)
        flag = ""
        if synthetic:
            flag = (
    ...
    cover = f"""
<div class="cover">
  <div class="kicker">完整深度扩写版 · 内部研究报告 / 与 JOH 投稿平行文档</div>
  <h1>全局 EOF 降维何时不足以支撑<br>多保真洪水淹没模拟？</h1>
  <p class="subtitle">水动力分区 LSG-Max：等模态预算、面积加权指标、训练期分区与机制诊断（stage-swap）</p>
  <div class="meta">
    <p>项目路径：{escape(PROJECT_PATH)}</p>
    <p>日期：{DATE_STR}　·　文档类型：完整中文学术研究报告（深度图解扩写）</p>
    <p>数据登记：<code>outputs/registry/result_manifest_v4.csv</code></p>
    <p>生成脚本：<code>{SCRIPT_NAME}</code>（旧版平行稿见 <code>研究报告.html</code>，本文件不覆盖）</p>
  </div>
</div>
"""
    toc = """
  ...
  <ol>
    ...
    <li><a href="#s3">数据与方法</a>
      <ol>
        ...
        <li><a href="#s3-5">二阶诊断与 stage-swap</a></li>
      </ol>
    </li>
    <li><a href="#s4">研究过程</a>
      <ol>
    ...
    <li><a href="#s5">结果展示</a>
      <ol>
        ...
        <li><a href="#s5-5">stage-swap 机制实验</a></li>
      </ol>
    </li>
    <li><a href="#s6">分析与讨论</a>
      <ol>
        <li><a href="#s6-1">分区何时有用（为何 Carlisle 有效）</a></li>
        <li><a href="#s6-2">全局为何随模态变差</a></li>
        <li><a href="#s6-3">为何 Burnett / Chowilla 不然</a></li>
        <li><a href="#s6-4">官方划分与 CSI</a></li>
        <li><a href="#s6-5">EOI 证伪、官方 9 折、外推、LF 加粗与主槽距离</a></li>
        <li><a href="#s6-6">stage-swap 揭示的机制</a></li>
      </ol>
    </li>
...
<p>残差组织指数在<strong>训练事件</strong>上计算：先对每个湿格子求多场平均 |LF − HF|，再按规则分区把格子归入各区。分子是各区均值的方差，分母是格子级方差。EOI 接近 0 表示残差像空间白噪声；EOI 高表示误差成块。<strong>但一阶 EOI 高不是分区获益的充分条件</strong>（见 Burnett）。最大淹没面 EOI 已三案齐全：Carlisle {eoi_c:.3f}、Chowilla {eoi_w:.3f}、Burnett {eoi_bmax:.3f}。历史时序 EOI（Carlisle {eoi_ts:.2f}）与最大面不是同一协议，仅作 SI 对照。</p>
{html_table(t_eoi_h, t_eoi_r, "表 R　残差组织。最大面 EOI 来自 residual_organization.csv / eoi_all.json；分区平均绝对残差（深槽 0.245 m / 边缘 0.010 m）引自 2026-06-07 v4 训练期，本次未重载 HDF5，不当作新实验。", "tblR")}
...
<div class="metrics">
  ...
  <div class="metric"><div class="v">{eoi_c:.3f}</div><div class="l">Carlisle 最大面 EOI（非开关）</div></div>
...
{html_table(
    ...
    [
        ["GG", "全局", "全局", fmt((d.get('stage_swap') or {}).get('loocv', {}).get('summary', {}).get('GG', {}).get('mean_rmse', float('nan'))), "基线"],
        ["ZZ", "分区", "分区", fmt((d.get('stage_swap') or {}).get('loocv', {}).get('summary', {}).get('ZZ', {}).get('mean_rmse', float('nan'))), "完整分区"],
        ["GZ", "全局（切区近似）", "分区", fmt((d.get('stage_swap') or {}).get('loocv', {}).get('summary', {}).get('GZ', {}).get('mean_rmse', float('nan'))), "≈ZZ"],
        ["ZG", "分区", "全局（拼接近似）", fmt((d.get('stage_swap') or {}).get('loocv', {}).get('summary', {}).get('ZG', {}).get('mean_rmse', float('nan'))), "≈ZZ"],
    ],
    "表 S　Carlisle B=4 stage-swap 九折 LOOCV 均值（stage_swap.json）。",
    "tblS",
)}
{expl.get('_stage_swap_html', '')}
<div class="kf"><strong>机制一句话。</strong>观测到 GZ≈ZG≈ZZ≪GG：收益来自分区结构进入耦合的表示→映射管线，而非纯 EOF 截断（oracle ΔRMSE&lt;0），也不能唯一归因于区私有 GP。</div>
</section>
"""
...
<section id="s6">
<h2>6　分析与讨论</h2>
<p>英文投稿稿的讨论分三问：分区何时提供价值、全局为何随额外模态变差、分区何时无必要甚至有害。本完整版在此基础上补上“来龙去脉”：为何 Carlisle 有效而 Burnett/Chowilla 不然；为何一阶 EOI 不能当开关；stage-swap 揭示了什么。所有数字仍只来自 Track B。</p>
<h3 id="s6-1">6.1　何时分区提供价值——为何 Carlisle 有效</h3>
<p>Carlisle 同时满足几件“合在一起才有用”的事，而不是单一指标触发开关：</p>
<ol>
<li><strong>LF 已经抓住主导地形。</strong>仅用 LF 的面积加权 RMSE 约 {fmt(LF)} m、CSI 约 {fmt(csi['lf'])}，说明粗网格 HEC-RAS 并没有完全画错洪水轮廓；LSG 要学的是可修正的系统偏差，而不是从零发明淹没。</li>
<li><strong>水深误差有可被水动力先验切开的结构。</strong>规则按最大水深与淹没频率切深槽/常淹/间歇/边缘，与河道—滩地力学一致；因此规则略优于 KMeans（{fmt(R4)} vs {fmt(K4)} m）。</li>
<li><strong>模态预算被公平收紧。</strong>在 B=4 时，全局必须用同一组模态同时迁就深槽与边缘；分区把总秩拆到更均匀的子问题。9/9 折 LOOCV 与事件 1 的灾难救援，说明这不是某一折运气。</li>
<li><strong>最大面一阶 EOI 并不高（{eoi_c:.3f}）。</strong>因此 Carlisle 的成功<strong>不能</strong>被叙述成“因为 EOI 高所以分区”。更贴切的说法是：在等预算与可修正的 LF–HF 偏差下，全局 EOF 的水动力非中性暴露出来了。</li>
</ol>
...
<p><strong>Burnett：高一阶 EOI，但等预算分区无益。</strong>最大面 EOI={eoi_bmax:.3f}，深槽与边缘残差可差到数米量级，误差极度成块。然而 30 折 LOOCV 上规则平均更差（{fmt(bloo['mean_zonal_rmse'])} vs {fmt(bloo['mean_global_rmse'])} m），仅 {bloo['n_improved']}/{bloo['n_folds']} 折占优，区间跨 0。解读：高 EOI 度量 LF 残差的空间组织；Burnett 表明强组织残差不足以保证等预算分区收益，现有诊断<strong>未唯一识别</strong>原因。12 事件单次划分上全局≈规则≈{fmt(b_g)} m，也说明“LSG 有用 ≠ 分区额外有用”。</p>
...
<p>把rival解释过一遍：</p>
<ul>
<li>偶然不等预算 → 已审计，真等预算；</li>
<li>检验泄漏进分区 → CLEAN_PASS；</li>
<li>收益=更好的 HF-EOF 截断 → oracle ΔRMSE&lt;0，排除；</li>
<li>收益=唯一来自区私有 GP → GZ≈ZZ 且 ZG≈ZZ，不支持“唯一”；</li>
<li>官方 2 折当主证据 → CI 含 0，拒绝。</li>
</ul>
<p>因此 Novelty 应写成：<strong>等预算条件性分区 + 一阶 EOI 证伪 + stage-swap 机制 + 诚实边界</strong>。勿写“首次 zonal LSG”（Tan et al., 2025 等已有区域化 LSG 训练先例）。</p>
</section>
"""
...
<li><strong>二阶 + stage-swap 机制：</strong>三案例 ZGG&gt;0 但等预算纯 EOF 的 ΔRMSE&lt;0（排除纯截断）。Carlisle LOOCV 均值 GG/ZZ/GZ/ZG ≈ 0.180/0.098/0.098/0.101 m，即 GZ≈ZG≈ZZ≪GG——收益来自分区结构进入表示→映射管线，不能唯一钉死在区私有 GP。</li>
<li><strong>Novelty 边界：</strong>等预算条件性分区 + EOI 证伪 + stage-swap 机制 + 诚实边界；勿写“首次 zonal LSG”。</li>
</ol>
</section>
"""
...
<ol>
<li>Fraehr, N., Wang, Q. J., Wu, W., &amp; Nathan, R. (2022). Upskilling low-fidelity hydrodynamic models of flood inundation through spatial analysis and Gaussian Process learning. <em>Water Resources Research</em>, 58, e2022WR032248. https://doi.org/10.1029/2022WR032248</li>
<li>Fraehr, N., Wang, Q. J., Wu, W., &amp; Nathan, R. (2023). Development of a fast and accurate hybrid model for floodplain inundation simulations. <em>Water Resources Research</em>, 59, e2022WR033836. https://doi.org/10.1029/2022WR033836</li>
<li>Fraehr, N., et al. (2024). Assessment of surrogate models for flood inundation: The physics-guided LSG model vs. state-of-the-art machine learning models. <em>Water Research</em>, 252, 121202. https://doi.org/10.1016/j.watres.2024.121202　公开基准 Figshare 24312658。</li>
<li>Fraehr, N., Wang, Q. J., Wu, W., &amp; Nathan, R. (2023). Supercharging hydrodynamic inundation models for instant flood insight. <em>Nature Water</em>, 1, 835–843. https://doi.org/10.1038/s44221-023-00132-2</li>
<li>Lu, J., Wang, Q. J., Fraehr, N., Xiang, X., &amp; Wu, X. (2025). Choice of Gaussian Process kernels used in LSG models for flood inundation predictions. <em>Journal of Hydrology</em>, 655, 132949. https://doi.org/10.1016/j.jhydrol.2025.132949</li>
<li>Tan, Z., et al. (2025). An efficient hybrid downscaling framework to estimate high-resolution river hydrodynamics. <em>Hydrology and Earth System Sciences</em>, 29, 3833–3852. https://doi.org/10.5194/hess-29-3833-2025　（区域化 LSG 先例；故勿称“首次 zonal LSG”）</li>
<li>Bentivoglio, R., et al. (2022). Deep learning methods for flood mapping. <em>HESS</em>, 26, 4345–4378. https://doi.org/10.5194/hess-26-4345-2022</li>
...
<title>完整研究报告：全局 EOF 降维何时不足以支撑多保真洪水淹没模拟？</title>
<style>
{CSS}
</style>
</head>
<body>
<div class="running">完整深度扩写版 · 水动力分区 LSG-Max · {DATE_STR} · 与 JOH 投稿平行 · 不覆盖 研究报告.html / report.html</div>
<div class="wrap">
{cover}
{toc}
{s1}
...
**生成脚本：** `{SCRIPT_NAME}`（图解库 `scripts/_deep_fig_zh.py`）
...
> **Burnett 30 折：分区没有帮忙。** 全局均值 {fmt(bloo['mean_global_rmse'])} m，规则 {fmt(bloo['mean_zonal_rmse'])} m，ΔRMSE = {fmt(bloo['mean_delta_rmse'])} m，{bloo['n_improved']}/{bloo['n_folds']} 折，区间 [{fmt(bloo['ci_95_lower'])}, {fmt(bloo['ci_95_upper'])}]，`significant=false`。最大面 EOI 却高达 {eoi_bmax:.3f}。
...
最大面 EOI：Carlisle {eoi_c:.3f}，Chowilla {eoi_w:.3f}，Burnett {eoi_bmax:.3f}。时序 EOI（Carlisle）为 {eoi_ts:.2f}。
...
湿润阈值 0.03 m；规则/KMeans 分区、EOF、GP 均只在训练事件上拟合；真等预算；面积加权；泄漏审计 CLEAN_PASS；二阶 ZGG + 纯 EOF oracle；stage-swap GG/ZZ/GZ/ZG。

{md_fig(1, "全局 LSG 与分区 LSG 的流程对照（示意图）", "fig01_workflow.png",
"左列全局 EOF+GP；右列先分区再分区 EOF+GP。示意图不含可引用 RMSE。逐子图深度解释见 HTML。", True)}
...
{md_fig(3, "Carlisle 真等预算 RMSE–B 曲线", "fig03_mode_budget.png",
...
{md_fig(6, "Carlisle 九折 LOOCV（B=4）", "fig08_per_event_bootstrap.png", "规则线在全部 9 个留出事件上低于全局。")}
...
{md_fig(9, "三案例 RMSE 柱状图", "fig04_three_case.png", "Carlisle 分区最好；Chowilla LSG 远高于 LF；Burnett 全局≈分区。")}
...
def main():
    ...
    stage_p = OUT / "carlisle" / "stage_swap.json"
    stage_swap = load_json(stage_p) if stage_p.exists() else {}
