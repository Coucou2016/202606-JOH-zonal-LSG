...
> **Burnett 30 折：分区没有帮忙。** 全局均值 1.7192 m，规则 1.8164 m，ΔRMSE = -0.0972 m，13/30 折，区间 [-0.2249, 0.0014]，`significant=false`。最大面 EOI 却高达 0.957。

> **机制。** 纯 EOF oracle ΔRMSE&lt;0；stage-swap LOOCV 均值 GG/ZZ/GZ/ZG ≈ 0.180/0.098/0.098/0.101 → GZ≈ZG≈ZZ≪GG。收益来自分区结构进入表示→映射管线。

Chowilla 为边界情形：LSG RMSE 约 2.5606 m，仅用 LF 为 0.3926 m。GP 后端为 sklearn GPR，非 gpflow SGPR【正式局限】。Novelty=等预算条件性分区+EOI 证伪+stage-swap+诚实边界；勿写“首次 zonal LSG”。
...
主图：Carlisle LOOCV 事件 1（Run2）。RMSE LF≈0.233 / 全局 0.6953 / 规则 0.1668 m。

### 图 S1　Run2 最大水深四联图

![Run2 最大水深四联图](outputs/figures/figA1_inundation_maps_carlisle_ev1.png)

HF/LF/Global/Rule；规则更接近 HF。

### 图 S2　Run2 命中—漏报—空报

![Run2 命中—漏报—空报](outputs/figures/figA2_csi_hitmiss_carlisle_ev1.png)

规则空报显著减少，CSI 升高。

### 图 S3　Run2 残差与改善

![Run2 残差与改善](outputs/figures/figA3_residuals_carlisle_ev1.png)

全局系统性偏深；规则残差近零。

### 图 S4　Run2 分区叠加

![Run2 分区叠加](outputs/figures/figA4_zones_overlay_carlisle_ev1.png)

训练期规则分区叠在 HF 水深上。

### 图 S5　Run2 obs–pred 散点

![Run2 obs–pred 散点](outputs/figures/figA5_obs_vs_pred_carlisle_ev1.png)

湿单元 1:1；规则更贴对角线。

### 图 S6　Run1 淹没四联图

![Run1 淹没四联图](outputs/figures/figA1_inundation_maps_carlisle_ev0.png)

较温和折，方向一致。

### 图 S7　Burnett 事件 0 淹没图

![Burnett 事件 0 淹没图](outputs/figures/figA1_inundation_maps_burnettrv_ev0.png)

Global≈Rule。

### 图 S8　Chowilla Chow_p01 淹没图

![Chowilla Chow_p01 淹没图](outputs/figures/figA1_inundation_maps_chowilla_ev0.png)
