...
where \(\bar{r}_k\) is the mean of \(\bar{r}_i(S)\) over the active cells in zone \(k\), \(\mathrm{Var}_k\) is the unweighted variance across active-zone means (each zone is weighted equally regardless of its cell count), and \(\mathrm{Var}_i\) is the spatial variance of \(\bar{r}_i(S)\) over the active mask. Because the numerator is not a cell-count-weighted between-group variance, EOI is not by construction confined to the unit interval. A high value indicates that residual magnitude is more strongly separated by the chosen zones. EOI is an exploratory diagnostic and is not calibrated as a prospective decision threshold. It is evaluated in two ways: as a retrospective pooled descriptor over all events in the analysis extract for each case (Figure 15), and as a strictly train-only value on each leave-one-out training fold (Figure 16).
