...
Code and evaluation artefacts: https://github.com/Coucou2016/202606-JOH-zonal-LSG (manuscript draft v1.0-rc).
...
We use the public Fraehr multi-fidelity inundation benchmark (Figshare 24312658; Table 1). Carlisle (UK; HF LISFLOOD-FP, LF HEC-RAS 2D) is the primary statistical case (all nine available events). Chowilla (Australia; MIKE 21) is treated as a boundary case where LF–HF mismatch can make LSG correction harmful; the 12-event subset follows the Fraehr multi-case split used for cross-case LSG comparisons rather than an author-selected cherry-pick, and 29 Chowilla events are available in total. Burnett River (Australia; HF TUFLOW, LF HEC-RAS 2D) appears in two protocols: a 12-event fixed split aligned with the same three-case table, and a separate 30-event leave-one-out contrast under the equal-B protocol. The 30-event LOOCV uses a fixed 30-event Burnett max-surface extract prepared for this repository rather than a randomly redrawn subset in this study; 74 events are available in the archive, and fuller Burnett extensions remain a stated limitation. Cell counts are those of the max-surface extracts analysed here (Carlisle HF/LF = 581,061 / 5,681; Chowilla = 109,914 / 1,434; Burnett = 780,785 / 15,256). The Carlisle and Chowilla high-fidelity counts match the benchmark description, whereas the Burnett high-fidelity grid is reported there as ≈3.7 million cells; the 780,785-cell Burnett field used here is the extracted evaluation subset rather than the full native grid, and Burnett results should be read on that basis. Geometric cell areas are taken from the Fraehr geometry archives: Carlisle and Burnett are uniform (25 m² and 400 m²), whereas Chowilla areas vary widely; area-weighted metrics therefore coincide with cell-count weighting on Carlisle and Burnett but remain informative on Chowilla. Synthetic toy grids are excluded from all claims. Brisbane River licensed data are not available in this study and are out of scope.
...
| Chowilla | Australia | MIKE 21 | 12 / 29 | 109,914 / 1,434 | Boundary: LSG degrades vs LF-only |
| BurnettRV | Australia | TUFLOW | 12 (three-case split); 30 (LOOCV) / 74 available | 780,785 / 15,256 | Contrast; Rule not favoured on LOOCV |
...
**What zoning changes.** Zoned LSG-Max first partitions wet cells, then runs per-zone EOF+GP under a shared total mode budget B. Rule zoning may label up to five hydrodynamic classes (deep/near-channel, frequent, intermittent, fringe, and an optional residual-error hotspot override); KMeans uses a fixed K = 4 in primary comparisons (K = 6 in configuration sweeps only). Mode allocation is applied over nonempty active zones after the partition: when B is at least the number of active zones, each active zone receives one base mode and any remainder is shared by zone variance; the headline Carlisle equal-B artefacts report realized total zonal modes equal to the requested budget (actual_modes = 4 at B = 4; 6 at B = 6). Rule features are training maximum depth, inundation frequency, and—when residual fields are supplied—an LF–HF mean-absolute-residual hotspot override (error percentile 80; deep percentile 80; frequent/intermittent frequency cut-offs 0.7/0.1). KMeans features are standardized on training active cells only.

**What is held equal.** Event splits, wet-depth threshold, evaluation fields, and total retained-mode capacity B are matched in the strict equal-B comparisons. Zoning features, EOF bases, and GPs are fit on training events only; test events enter only at prediction and evaluation. Headline results are LSG-Max; real-data zonal LSG-TS is not claimed here. A sparse GPflow path is implemented in the public code base but was not executed in the reported environment; backend equivalence is therefore a formal limitation rather than a missing hyperparameter. Figure 1 summarizes the equal-budget workflow; Figure 2 shows representative train-only zone maps.
...
Results are presented from the spatial fields upward: held-out inundation and error maps first, then equal-budget Carlisle skill curves, event-level paired stability, three-case boundaries, mechanism diagnostics, and secondary robustness checks. The maps are diagnostic illustrations of individual folds and are not pooled skill estimates; the quantitative subsections that follow carry the statistical claims.

### 3.1 Qualitative spatial diagnosis

Before aggregating skill into curves and confidence intervals, we examine the predicted fields directly. Each map in this subsection comes from a single held-out leave-one-out fold at B = 4, so the panels show where global and zoned representations differ spatially rather than how large the difference is on average.

**Carlisle large-error fold.** Event index 1 (Run2) is the LOOCV fold with the largest Global RMSE spike at B = 4 and is shown as a diagnostic extreme rather than as a pooled skill estimate. Held-out LOOCV area-weighted RMSE on this fold is LF 0.233 m, Global 0.694 m, and Rule 0.166 m. Side-by-side maximum-depth maps (common colour scale) show that Global LSG-Max introduces floodplain artefacts relative to HF, whereas Rule recovers a visually closer depth field (Fig. 3).

![Fig. 3. Carlisle Run2 held-out max-depth maps (HF, LF, Global, Rule) under B = 4 LOOCV; common colour scale.](../outputs/figures/figA1_inundation_maps_carlisle_ev1.png)

Wet/dry hit–miss maps (threshold 0.03 m) locate the CSI gap: Global CSI = 0.591 versus Rule CSI = 0.816 on the same held-out Run2 surface, with false-alarm cells markedly reduced under Rule (Fig. 4).

![Fig. 4. Carlisle Run2 hit–miss maps at 0.03 m for Global and Rule under B = 4 LOOCV.](../outputs/figures/figA2_csi_hitmiss_carlisle_ev1.png)

Residual panels (Global−HF, Rule−HF, and absolute-error improvement) show a broad over-deep Global bias that Rule largely removes across the wet floodplain (Fig. 5).

![Fig. 5. Carlisle Run2 residual and improvement maps (Global−HF, Rule−HF).](../outputs/figures/figA3_residuals_carlisle_ev1.png)

Rule zones from the train-only fit are overlaid on the HF depth field to link the partition to the inundation pattern (Fig. 6). Wet-cell observed-versus-predicted scatter complements the maps with a cell-level 1:1 view (Fig. 7).

![Fig. 6. Carlisle Run2 train-only Rule zones overlaid on HF depth.](../outputs/figures/figA4_zones_overlay_carlisle_ev1.png)

![Fig. 7. Carlisle Run2 wet-cell observed versus predicted depths for Global and Rule.](../outputs/figures/figA5_obs_vs_pred_carlisle_ev1.png)
...
At B = 4, Global / Rule / KMeans area-weighted RMSE = 0.1464 / 0.0964 / 0.1015 m (Rule −34.2% versus Global). Across the retained-mode budget sweep, Global RMSE increased from 0.1464 to 0.2588 and 0.3527 m, while Rule increased from 0.0964 to 0.1256 and 0.1790 m. B = 4 and 6 are strict matched-budget comparisons; the nominal Global B = 8 point realized seven modes and is retained only as a capacity-mismatch exception. Fig. 8 therefore supports a capacity-sensitive Global curve under this protocol rather than a claim that more modes always help. Component-level attribution (for example, event-noise fitting) is not uniquely identified here. CSI does not show a matching zonal win: LF-only CSI (0.9145) exceeds LSG CSI at B = 4, so zoning improves depth RMSE more than wet–dry extent (Figs. 8, 9, and 10).

![Fig. 8. Carlisle area-weighted depth RMSE versus retained-mode budget B for Global, Rule, and KMeans LSG-Max (strict equal-B at B = 4 and 6; B = 8 shown as a capacity mismatch).](../outputs/figures/fig03_mode_budget.png)

![Fig. 9. Carlisle area-weighted CSI versus retained-mode budget B (wet threshold 0.03 m).](../outputs/figures/fig09_csi_budget.png)

![Fig. 10. Carlisle area-weighted MAE and bias versus retained-mode budget B.](../outputs/figures/fig13_mae_bias.png)

### 3.3 Event-level statistics

Carlisle B = 4 LOOCV: 9/9 folds improved; mean ΔRMSE = 0.0821 m; 95% CI [0.0155, 0.1987]. B = 6: 7/9; mean 0.0606 m; CI [0.0032, 0.1618]. Official two-fold: mean ΔRMSE = 0.0045 m; CI [−0.0073, 0.0134]; significant = false. These protocols answer different questions: LOOCV tests fold-consistent paired improvement under equal B, whereas the official two-fold split is a coarser sensitivity check with wide uncertainty and must not be read as contradicting 9/9 LOOCV. Burnett B = 4 30-fold: mean Global 1.7479 m versus Rule 1.8260 m; ΔRMSE = −0.0781 m; 6/30; significant = false. The Burnett 12-event fixed split in Table 2 is a different protocol (Global ≈ Rule) and is not interchangeable with the 30-fold LOOCV non-benefit contrast (Figs. 11–14).

![Fig. 11. Carlisle per-event paired ΔRMSE under B = 4 LOOCV with bootstrap uncertainty.](../outputs/figures/fig08_per_event_bootstrap.png)

![Fig. 12. Carlisle LOOCV Global versus Rule RMSE scatter at B = 4.](../outputs/figures/fig11_loocv_scatter.png)

![Fig. 13. Bootstrap confidence intervals for mean paired ΔRMSE (Carlisle LOOCV and official two-fold; Burnett 30-fold).](../outputs/figures/fig12_stat_ci.png)

![Fig. 14. Burnett 30-fold LOOCV Global versus Rule RMSE at B = 4.](../outputs/figures/fig10_burnett_loocv.png)
...
![Fig. 15. Three-case area-weighted RMSE at B = 4 for LF-only, Global, and Rule (Table 2).](../outputs/figures/fig04_three_case.png)

### 3.5 EOI, second-order, and stage-swap

Max-surface EOI: Carlisle 0.057, Chowilla 0.116, Burnett 0.957. Zoning benefit does not increase with EOI: the clearest equal-B gain occurs at the lowest EOI, while the highest EOI coincides with no Rule advantage. That pattern shows that the tested EOI ordering does not explain the observed case ordering, rather than establishing an operational threshold or proving that no residual-organization metric could ever help. Modal diagnostics show ZGG > 0 with equal-budget pure-EOF oracle ΔRMSE < 0 on all three cases, ruling out the claim that zoning helps merely by truncating HF EOF better. Area-weighted oracle ΔRMSE remains negative on Chowilla (−0.0543 versus −0.0657 unweighted) and is identical to the unweighted value on Carlisle and Burnett (uniform areas), so weighting does not rescue a truncation-only story. Stage-swap LOOCV means: GG ≈ 0.180, ZZ ≈ 0.098, GZ ≈ 0.098, ZG ≈ 0.101 m. Because GZ ≈ ZG ≈ ZZ ≪ GG, either approximate localization of the EOF coordinates or of the GP stack recovers most of the ZZ gain; the design therefore rejects single-stage necessity claims without uniquely identifying one localized stage as the mechanism. Complementary EOI and modal diagnostics are shown in Figs. 16, 17, and 18.

![Fig. 16. Max-surface EOI by case (Carlisle, Chowilla, Burnett).](../outputs/figures/fig14_eoi.png)

![Fig. 17. EOI versus equal-B zoning ΔRMSE across cases.](../outputs/figures/fig15_eoi_vs_delta.png)

![Fig. 18. Modal EOI / ZGG diagnostics complementary to first-order EOI.](../outputs/figures/fig19_modal_eoi.png)

### 3.6 Secondary sensitivity and robustness analyses

This subsection collects protocol-level secondary sensitivity analyses that are secondary to the equal-B LOOCV claim and are not head-to-head comparisons with a locally reproduced LSG-TS implementation (Fig. 19). On the official nine-fold MaxWD R² protocol, Rule LSG-Max reaches 0.988 versus published LSG-TS 0.990 (Global Max ≈ 0.915). LF coarsening and channel-distance zoning probes (Figs. 20–21) support robustness narratives but do not redefine the primary equal-B result.

![Fig. 19. Official MaxWD R² protocol contrast (sanity check only).](../outputs/figures/fig16_official_maxwd_r2.png)

![Fig. 20. LF coarsening probe (secondary robustness).](../outputs/figures/fig17_lf_degradation.png)

![Fig. 21. Channel-distance zoning probe (secondary robustness).](../outputs/figures/fig18_channel_distance.png)
...
Rival explanations rejected or weakened include: (i) accidental unequal budgets (matched-capacity checks); (ii) test leakage into zoning (train-only fits and leakage probes); (iii) GP-only localization as the unique cause (ZG ≈ ZZ as well as GZ ≈ ZZ); and (iv) official two-fold as primary evidence (CI includes zero).
...
**Decision transfer.** No transferable operational zoning selector is claimed. Prospective selector validation would require a pre-registered diagnostic, a hold-out case protocol, and success/failure criteria defined before looking at test metrics; that experiment is not claimed here.
...
2. In the Carlisle budget sweep, increasing the requested global mode count did not recover the B = 4 zonal advantage; the nominal B = 8 point is retained as a capacity mismatch (Global realized 7 modes) rather than a strict equal-B comparison.
